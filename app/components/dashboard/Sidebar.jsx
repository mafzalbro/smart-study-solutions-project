...
export default function Sidebar({ height }) {
  ...
  return (
    ...
    // -translate-x-96
    <>
    <div className={`w-0 overflow-hidden p-0 md:w-1/3 bg-neutral-50 md:bg-transparent dark:bg-accent-800 dark:md:bg-transparent ${!isSidebarVisible ? 'w-0 overflow-hidden p-0 opacity-0 md:opacity-100 relative': 'opacity-100 overflow-auto p-4 fixed md:relative h-full md:h-[100vh] md:w-1/3 w-full z-20'} md:w-1/4 md:border-r-2 md:dark:border-accent-900 md:p-4 flex flex-col gap-10 transition-all ease-in-out duration-500`}>
    ...
    <button onClick={toggleSidebar} className={`fixed transition-all ease-in-out duration-400 rounded-full m-4 p-2 bg-accent-50 z-20 text-primary ${!isSidebarVisible ? 'left-1' : 'left-[45%]'} md:hidden`}>
