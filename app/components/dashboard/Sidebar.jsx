...
export default function Sidebar({ height }) {
  ...
  return (
    <>
      <div
        ref={scrollRef}
        className={`md:flex-col md:gap-4 flex flex-row gap-2 md:p-4 md:w-1/3 overflow-x-auto overflow-y-hidden whitespace-nowrap scrollbar-hide rounded-lg bg-gray-100 dark:bg-neutral-800 md:bg-transparent md:dark:bg-transparent my-8 w-[90vw] mx-auto md:mx-0 md:my-0 pl-2 py-2`}
          ...
          <div className='flex md:flex-col'>
            {sidebarItems.map((item) => (
              <Link key={item.key} href={item.href}>
                <span className={`block py-2 px-4 mr-2 md:mr-0 md:my-2 rounded-lg ${selectedData === item.key ? 'bg-accent-500 text-secondary' : 'bg-secondary dark:bg-neutral-700 text-neutral-400 hover:bg-neutral-50 dark:hover:bg-accent-800 md:bg-transparent md:dark:bg-transparent'}`}>
                  {item.label}
                </span>
              </Link>
            ))}
