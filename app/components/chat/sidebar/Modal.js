  ...
  return modalVisible ? (
    <div className="fixed top-0 left-0 w-full flex items-center justify-center bg-primary bg-opacity-50 z-50">
      <div
        ...
        style={{
          top: modalPosition.y / 1.3,
          left: modalPosition.x * 1.1,
          position: "absolute",
        }}
      >
        <button
          onClick={() =>
            handleEdit({
              slug: selectedChatSlug,
              title: (activeTab === "chat" ? chats : pdfChats).find(
                (chat) => chat.slug === selectedChatSlug
              )?.title,
            })
          }
        ...
        {activeTab === "pdf" &&
          pdfChats.find((chat) => chat.slug === selectedChatSlug)
            ?.pdfUrls[0] && (
            <Link
              href={`${
                pdfChats.find((chat) => chat.slug === selectedChatSlug)
