          ...
          <>
            {pdfUrls.length > 0 && (
              <div className="bg-secondary dark:bg-neutral-700 p-4 rounded-lg mb-4">
                <h3 className="text-lg text-primary dark:text-neutral-100">
                  PDF URLs You Provided In This Chat
                </h3>
                {pdfUrls.map((pdfUrl, index) => (
                  <div key={index} className="mt-2">
                    <a
                      ...
                      className="dark:text-accent-300 dark:hover:text-accent-400 text-accent-500 hover:text-accent-600 inline-flex items-center"
                    >
            ...
            {chatHistory?.length > 0 ? (
              ...
              <ChatMessage
                display="flex justify-center flex-col items-center h-[75vh] text-lg text-neutral-400"
                key={Date.now()}
                message={{
                  user_query: "Hey, ",
                  model_response: "Empty Message...",
                }}
              />
            )}
          </>
