...
const changeExpirationTime = (
  key,
  time = 60000,
  slugs = ["/api/auth/check-auth", "/api/user/get/one"]
) => {
...
// Clear all older items from cache
export const removeOlderCacheAfterMutation = async (url) => {
  const cache = await caches.open(CACHE_NAME);
  const keys = await cache.keys();
  ...
  try {
    ...
    // If parsing fails, skip and continue
    console.error(`Error removing cache item for key: ${request.url}`, e);
  }
};
