...
import {
  ...
  FiChevronDown,
  FiChevronUp,
} from "react-icons/fi"; // Added icons for toggling
import {
  BiSolidUpvote,
  BiUpvote,
  BiDownvote,
...
const QuestionPage = ({ params }) => {
  ...
  const handleReportSubmit = () => {
    ...
    fetcher(
      `${process.env.NEXT_PUBLIC_BACKEND_ORIGIN}/api/qna/question/${slug}/report`,
      "POST",
  ...
  const handleDownvote = () => {
    ...
    fetcher(url, "POST")
      ...
      .catch((error) => {
        ...
        setAlertMessage({
          message: `Error downvoting question.`,
          type: "error",
        });
      });
  };
        ...
        <div className="main-qna-container">
              ...
              <div>
                {question?.askedBy?.fullname && (
                  <p className="font-semibold">
                    {question?.askedBy?.fullname === user?.fullname
                      ? question?.askedBy?.fullname + " (You)"
                      : question?.askedBy?.fullname}
                  </p>
                )}
          ...
          <div className="p-5 my-10 rounded-lg bg-neutral-50 dark:bg-neutral-950 dark:text-neutral-100 shadow-lg">
            <div className="flex items-center space-x-4">
              ...
              <div
                className="flex items-center cursor-pointer hover:text-accent-600"
          ...
          {Array.isArray(question?.answers) && question?.answers?.length > 0 ? (
            question?.answers?.map((answer, index) => (
              <div
                key={index}
                className={`relative mb-4 p-8 pt-0 rounded-lg dark:bg-neutral-950 dark:text-neutral-100 border border-neutral-300 dark:border-neutral-800 transition-all duration-300 ${
                  expandedAnswers[index] ? "h-full" : "h-auto"
                }`}
              >
                <div
                  className="flex justify-between cursor-pointer pt-8"
                ...
                {expandedAnswers[index] && (
                  <div
                    className={`transition-all duration-400 ease-in-out ${
                      expandedAnswers[index] ? "-top-10" : "top-0"
                    }`}
                  >
                    ...
                    <div className="mb-2">
                      {answer?.answeredBy && (
                        <p className="font-semibold my-4">Answered by:</p>
                      )}
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          {answer?.answeredBy?.fullname && (
                            <p className="font-semibold">
                              {answer?.answeredBy?.fullname === user?.fullname
                                ? answer?.answeredBy?.fullname + " (You)"
                                : answer?.answeredBy?.fullname}
                            </p>
                          )}
      ...
      {isAnswerModalOpen && (
        ...
        >
          <AnswerForm
            ...
            onSuccess={handleAnswerModalClose}
            onClose={handleAnswerModalClose}
          />
        </AnswerModal>
      )}
