...
import { AiOutlineHome, AiOutlineEdit, AiOutlineLock, AiOutlineHeart, AiOutlineQuestion } from "react-icons/ai";
...
const quickLinks = [
  ...
  { label: "Update Profile", href: "/dashboard/profile", key: "profile", icon: <AiOutlineEdit /> },
...
export default function UserDashboard() {
  ...
  return (
    <div className="flex flex-col gap-10 mt-10">
      {/* Quick Links Section */}
      <StylishTitle className="text-white text-center" fontSize="3xl" tagName="h2" colored="Quick Links" />
        ...
        {quickLinks.map((link) => (
          <Link key={link.key} href={link.href} passHref>
            <div className="group w-40 p-4 bg-white dark:bg-neutral-800 rounded-lg shadow-lg border border-neutral-200 dark:border-neutral-700 hover:shadow-xl transition-transform transform hover:-translate-y-1 flex flex-col items-center justify-center text-center">
              <div className="text-3xl mb-2 text-accent-500 group-hover:text-accent-700">{link.icon}</div>
      ...
      {/* Profile Section */}
      <StylishTitle className="text-white text-center" fontSize="3xl" tagName="h2" colored="User Profile" />
        ...
        <div className="relative w-full max-w-md p-6 bg-my-bg-2 rounded-lg border border-neutral-100 dark:border-neutral-700 mx-4 bg-secondary dark:bg-neutral-800">
          <div className="absolute -top-12 left-1/2 transform -translate-x-1/2">
            <div className="relative w-24 h-24">
              {user?.profileImage ? (
                <img src={user.profileImage} alt="Profile Picture" className="w-full h-full rounded-full border-2 border-accent-500" />
              ) : (
                <FaUserCircle size={96} className="text-neutral-400 dark:text-neutral-600" />
              )}
            </div>
          </div>
          <div className="pt-12 pb-6 text-center">
            <h2 className="text-3xl font-bold mb-1">{user?.fullname || "Dear User"}</h2>
            <p className="text-md text-neutral-600 dark:text-neutral-300">{user?.email || "your-email@example.com"}</p>
          </div>
