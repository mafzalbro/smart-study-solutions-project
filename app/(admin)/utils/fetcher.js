...
import { clearExpiredCache, getCachedData, setCachedData, removeUserCacheHistory } from '@/app/(admin)/utils/caching'
...
export const fetcher = async (url, method = 'GET', body = null, headers = {}) => {
  ...
  if (response.status === 401 && !path.includes('/admin/login')) {
    ...
    window.location.replace(`${process.env.NEXT_PUBLIC_FRONTEND_ORIGIN}/admin/login`);
  } else if (response.status === 200 && !token && path.includes('/admin/login')) {
