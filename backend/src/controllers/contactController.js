...
const sendNewsletter = async (req, res) => {
  try {
    ...
    const subscribedContacts = await Contact.find({ subscribed: true });

    if (!subscribedContacts.length) {
      return res
        .status(404)
        .json({ success: false, message: "No subscribed users found" });
    }

    // Send the newsletter to all subscribed users
    const emailPromises = subscribedContacts.map((contact) =>
      sendGenericEmail(contact.email, subject, null, htmlContent)
    );
    ...
    res.status(200).json({
      success: true,
      message: `Newsletter sent to ${subscribedContacts.length} subscribed users`,
    });
  } catch (error) {
